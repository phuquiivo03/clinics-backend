...
import { dayPackageService } from "../services";
import { CustomExpress } from "../pkg/app/response";
import { ErrorCode } from "../pkg/e/code";
import type { DayPackage } from "../types/dayPackage";
import type { ObjectId } from "mongoose";
import mongoose from "mongoose";
import { createDayPackageSchema, findDayPackageByIdSchema, updateDayPackageSchema } from "../schemas";
...
const create: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        const dayPackageData: DayPackage = {
            day_offset: validatedData.day_offset,
            period_pkgs: validatedData.period_pkgs.map(id => id as unknown as ObjectId)
        };
...
const update: RequestHandler = async (req, res, next) => {
    ...
    try {
        ...
        if (data.period_pkgs && Array.isArray(data.period_pkgs)) {
            data.period_pkgs = data.period_pkgs.map(id => {
                if (typeof id === 'string') {
