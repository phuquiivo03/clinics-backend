...
export const checkOwnerOrRole = (model: ModelRelate, documentIdField: string,  field: string, role: ROLE[]) => async (req: Request, res: Response, next: NextFunction) => {
  ...
  const document = await currentModel.findById(documentId);
    ...
    if (document[field].toString() !== user._id.toString() && !role.includes(user.role)) { // Check if the user is the owner or has the required role
