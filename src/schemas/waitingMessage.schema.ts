...
export const createWaitingMessageSchema = z.object({
  ...
  status: z
    .enum([WaitingMessageStatus.PENDING, WaitingMessageStatus.READ, WaitingMessageStatus.DELETED], {
...
export const updateWaitingMessageSchema = z.object({
  status: z.enum(
    [WaitingMessageStatus.PENDING, WaitingMessageStatus.READ, WaitingMessageStatus.DELETED],
    {
      message: 'Status must be either "pending", "read", or "deleted"',
    },
  ),
