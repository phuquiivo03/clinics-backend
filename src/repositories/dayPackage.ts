...
class DayPackageRepositoryImpl extends BaseRepositoryImpl<DayPackage> implements DayPackageRepository {
    constructor() {
    ...
    async findByDayPkgId(dayPkgId: ObjectId): Promise<DayPackage | null> {
        try {
            return this.model.findOne({ day_pkg_id: dayPkgId })
                .populate('period_pkgs')
                .exec();
        } catch (error) {
            throw error;
        }
