...
class OTPService {
  ...
  async create(email: string) {
    try {
      const existedOtp = await this.otpRepository.findOne({ filter: { email } });
      if (existedOtp) {
        await this.otpRepository.delete(existedOtp._id);
      }
      const code = Math.floor(100000 + Math.random() * 900000).toString();
      const otp = await this.otpRepository.create({ code, email });
      return otp;
    } catch (e) {
      throw e;
    }
  ...
  async verify(email: string, code: string): Promise<boolean> {
    try {
      const otp = await this.otpRepository.findOne({ filter: { email, code } });
      console.log(otp);
      if (!otp) {
        return false;
      }
